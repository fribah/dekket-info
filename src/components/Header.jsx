...
export default function Header() {
  ...
  const goToCard = (id) => {
    setShowNav(false);
    if (location.pathname !== "/forsikringer") {
      navigate(`/forsikringer?card=${id}`);
    } else {
      setTimeout(() => {
  ...
  return (
    <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 24px", maxWidth: 920, margin: "0 auto", position: "relative" }}>
      <a href="/" style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none" }}>
        <Logo size={26} /><span style={{ fontSize: 16, fontWeight: 800, letterSpacing: "-0.02em", color: "#111", fontFamily: FF }}>dekket.info</span>
      </a>
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <a href="/forsikringer" style={{ fontSize: 13, fontWeight: 600, color: "#555", textDecoration: "none", fontFamily: FF }}>Finn mine forsikringer</a>
        <div style={{ position: "relative" }}>
          {isMobile ? (
            ...
            <button onClick={() => setShowNav(!showNav)} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 13, fontWeight: 600, color: "#555", background: "transparent", border: "none", padding: "7px 0", cursor: "pointer", fontFamily: FF }}>
              Forsikringer <span style={{ display: "flex", transform: showNav ? "rotate(180deg)" : "", transition: "0.15s" }}><ChevIcon /></span>
            </button>
          )}
          {showNav && (<>
            ...
            <div style={{ position: isMobile ? "fixed" : "absolute", ...(isMobile ? { top: 0, right: 0, bottom: 0, width: 260, background: "white", zIndex: 100, padding: "60px 0 20px", boxShadow: "-4px 0 20px rgba(0,0,0,0.1)" } : { right: 0, top: "calc(100% + 4px)", background: "white", border: "1px solid #E5E5E5", borderRadius: 10, padding: "6px 0", minWidth: 220, boxShadow: "0 8px 30px rgba(0,0,0,0.08)", zIndex: 100 }) }}>
              {isMobile && <button onClick={() => setShowNav(false)} style={{ position: "absolute", top: 14, right: 14, background: "none", border: "none", cursor: "pointer", color: "#999" }}><CloseIcon /></button>}
              {NAV_ITEMS.map(item => (
